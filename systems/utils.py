...
def updateStateName(state_dict):
    new_state_dict = OrderedDict()
    for k, v in state_dict.items():
        if not 'occupancy_grid' in k:
            name = k[6:] # remove `module.`   'model.'
...
def mergeState(state_dict1, state_dict2, rate = 0.0):
    new_state_dict = {name:
                      state_dict1[name] * (1 - rate) + state_dict2[name] * rate
                      for name in state_dict1 if not 'occupancy_grid' in name
                      }
...
class PatchSampler():
    ...
    # def __init__(self, num_patch=4, patch_size=32, ratio_mask=1.0):
        self.n = num_patch
