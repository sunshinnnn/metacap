...
from tools.utils.mixins import SaverMixin
from tools.utils.misc import config_to_primitive, get_rank
import torch
from .utils import updateStateName
...
class BaseSystem(pl.LightningModule, SaverMixin):
    ...
    def __init__(self, config):
        ...
        if not self.config.model.ckpt is None:
            state_dict = torch.load(self.config.model.ckpt, map_location=lambda storage, loc: storage)['state_dict']
            state_dict = updateStateName(state_dict)
            self.model.load_state_dict(state_dict, strict = False)
            print("\n[DEBUG] load ckpt from {}\n".format(self.config.model.ckpt))
        # self.model = models.make(self.config.model.name, self.config)
