...
from .utils import updateStateName, mergeState
from pytorch3d.loss import chamfer_distance

@systems.register('neusinterpolation-system')
class NeuSSystem(BaseSystem):
    ...
    def prepare(self):
        ...
        self.state1 = updateStateName(torch.load(self.config.model.ckpt1, map_location=lambda storage, loc: storage)['state_dict'])
        self.state2 = updateStateName(torch.load(self.config.model.ckpt2, map_location=lambda storage, loc: storage)['state_dict'])
    ...
    def test_step(self, batch, batch_idx):
        stateMerged = mergeState(self.state1, self.state2, rate= float(int(batch['index'])/len(self.dataset) ) )
        self.model.load_state_dict(stateMerged, strict=False)
