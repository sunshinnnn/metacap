...
class SMPLlayer(nn.Module):
    def __init__(self, model_path, model_type='smpl', gender='neutral', device=None,
        ...
        **kwargs) -> None:
        ...
        if self.model_type == 'smplh':
            ...
            # mano_path = join(model_path,'mano')
            mano_path = r'E:\sungx\01_SMPL\smpl_family\mano'
    ...
    #         else:
    #             vertices = vertices[:, self.J_regressor.shape[0]:, :]
    #         # smpl_range = np.arange(22, 24 + 25).tolist() + [22,23]
    #         # vertices = vertices[:, smpl_range, :]
    #     vertices = torch.matmul(vertices, rot.transpose(1, 2)) + transl
    #     if not return_tensor:
    #         vertices = vertices.detach().cpu().numpy()
    #     return vertices

    def forward(self, poses, shapes=None, Rh=None, Th=None, scales=None, expression=None,
                v_template=None,
                return_verts=True, return_tensor=True, return_smpl_joints=False, true_smpl_joints = False,
                only_shape=False, pose2rot=True, **kwargs):
        """ Forward pass for SMPL model
        ...
        if 'torch' not in str(type(poses)):
            ...
            if scales is not None:
                scales = to_tensor(scales, dtype, device)
        ...
        bn = poses.shape[0]
        if poses.shape[1] == 69:
            poses = torch.cat( (   torch.zeros(bn, 3, dtype= poses.dtype,device=poses.device)   , poses )  , 1  )
        # process Rh, Th
        if scales is None:
            scales = torch.ones(bn, 1, device=poses.device)
        if shapes is None:
            shapes = torch.zeros(bn, 10, device=poses.device)
        if Rh is None:
            Rh = torch.zeros(bn, 3, device=poses.device)
        if Th is None:
            Th = torch.zeros(bn, 3, device=poses.device)
        ...
        if shapes.shape[0] < bn:
            shapes = shapes.expand(bn, -1)
        if scales.shape[0] < bn:
            scales = scales.expand(bn, -1)
        ...
        # if return_verts or not self.use_joints:
        if return_verts or true_smpl_joints:
            if v_template is None:
                v_template = self.v_template
            vertices, joints = self.lbs(shapes, poses, v_template,
                                        self.shapedirs, self.posedirs,
                                        ...
                                        use_pose_blending=self.use_pose_blending, use_shape_blending=self.use_shape_blending, J_shaped=self.J_shaped)
            # if not self.use_joints and not return_verts:
            if return_smpl_joints and true_smpl_joints and not return_verts:
                vertices = joints
        else:
            vertices, joints = self.lbs(shapes, poses, self.j_v_template,
                                        self.j_shapedirs, self.j_posedirs,
                                        ...
                                        self.j_weights, pose2rot=pose2rot, dtype=self.dtype, only_shape=only_shape,
                                        use_pose_blending=self.use_pose_blending, use_shape_blending=self.use_shape_blending, J_shaped=self.J_shaped)
            if return_smpl_joints and not true_smpl_joints:
                vertices = vertices[:, :self.J_regressor.shape[0], :]
            else:
                vertices = vertices[:, self.J_regressor.shape[0]:, :]

        vertices *= scales.unsqueeze(dim=1)
        # joints *= scales.unsqueeze(dim=1)
