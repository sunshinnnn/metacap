...
if __name__ == '__main__':
    ...
    for subj in subjectList:
        ...
        fileList = glob.glob(
            checkPlatformDir(
                '{}/{}/neus-domedense-{}-*-test/Cano_Meta_DDC@*/save/T_w2s.npz'.format(
                    baseDir, subj, subj)
                ))
        ...
        for item in tqdm(fileList):
            ...
            pathOut = osp.join(outDir, subj, clothType, "MetaCap", str(fIdx).zfill(6))
            makePath(pathOut)
            logger.info(pathInTemp + ' |  | ' + pathOut)
            for i in range(6):
                pathIn = osp.join(pathInTemp, imgFolder)
                copyFile(pathIn, pathOut, '{}.png'.format(i) , '{}.png'.format(i))
