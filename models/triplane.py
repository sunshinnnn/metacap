...
# from models.third_party.ops import grid_sample
...
#     # projections = torch.bmm(coordinates, inv_planes.to())
#     projections = torch.bmm(coordinates, inv_planes.to(coordinates.device))
...
class TriPlane(nn.Module):
    def __init__(self, in_channels, config):
        ...
        resX, resY, resZ = self.resolution[0], self.resolution[2], self.resolution[2]
        self.plane_xy = nn.Parameter(torch.randn(1, self.n_feature, resX, resY) * 0.001)
        self.plane_xz = nn.Parameter(torch.randn(1, self.n_feature, resX, resZ) * 0.001)
        self.plane_yz = nn.Parameter(torch.randn(1, self.n_feature, resY, resZ) * 0.001)
        self.n_output_dims = 3 * self.n_feature
    ...
    def forward(self, x):
        ...
        feat_xy = grid_sample(self.plane_xy, coords[..., [0, 1]])[0, :, :, 0].transpose(0, 1)
        feat_xz = grid_sample(self.plane_xz, coords[..., [0, 2]])[0, :, :, 0].transpose(0, 1)
        feat_yz = grid_sample(self.plane_yz, coords[..., [1, 2]])[0, :, :, 0].transpose(0, 1)
        feat = torch.cat([feat_xy, feat_xz, feat_yz], dim=1)
        feat = feat.reshape(*shape[:-1], 3 * self.n_feature)
        return feat
