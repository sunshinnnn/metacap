...
# import mytinycudann as tcnn

from pytorch_lightning.utilities.rank_zero import rank_zero_debug
from  models.triplane import TriPlane
from tools.utils.misc import config_to_primitive, get_rank
...
class CompositeEncoding(nn.Module):
    def __init__(self, encoding, include_xyz=False, include_feat= False,  xyz_scale=1., xyz_offset=0.):
        ...
        self.n_output_dims = int(self.include_xyz) * self.encoding.n_input_dims + self.encoding.n_output_dims + int(self.include_feat) * 7
    
    def forward(self, x, feat = None, *args):
        output = [self.encoding(x,  *args)]
        if self.include_xyz:
            output = [x * self.xyz_scale + self.xyz_offset] + output
        if self.include_feat and not feat is None:
            output = [feat] + output
...
def get_encoding(n_input_dims, config):
    ...
    elif config.otype == 'TriPlane':
        encoding = TriPlane(n_input_dims, config_to_primitive(config))
    else:
        with torch.cuda.device(get_rank()):
            encoding = tcnn.Encoding(n_input_dims, config_to_primitive(config))
    encoding = CompositeEncoding(encoding, include_xyz=config.get('include_xyz', False), include_feat=config.get('include_feat', False),\
                                 xyz_scale=2., xyz_offset=-1.)
    return encoding
